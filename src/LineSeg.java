    ...
    public class LineSeg
{
    ...
    public boolean parallelTo(LineSeg other)
    {
        //return true if this slope is the same as the other slope, or else within a very small number (EQUALITYDELTA)
        return MathFunctions.doubleEquals(this.slope(), other.slope(), Point.EQUALITYDELTA); 
    }

    public boolean perpendicularTo(LineSeg other) 
    {
        //if any of these 3 are true, return true, else return false:
        //1) this LineSeg slope is NOSLOPE (vertical) and the other is 0 (horizontal)   OR
        //2) this LineSeg slope is 0 (horizontal) and the other is NOSLOPE (vertical)   OR
        //3) the product of the slopes is -1.  For example 2/3 is a perpendicular slope to -3/2 and (2/3) * (-3/2) == -1
        return (MathFunctions.doubleEquals(this.slope(), LineSeg.NOSLOPE, Point.EQUALITYDELTA) && MathFunctions.doubleEquals(other.slope(), 0, .01)
                || MathFunctions.doubleEquals(this.slope(), 0, .01) && MathFunctions.doubleEquals(other.slope(), LineSeg.NOSLOPE, .01)
                || MathFunctions.doubleEquals(this.slope() * other.slope(), -1, .01));
    }
